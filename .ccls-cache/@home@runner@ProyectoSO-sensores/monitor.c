...
sem_t temperatura_vacia;     // Indicar que está vacio el buffer de temperatura
sem_t ph_completa;           // Indicar que está lleno el buffer de ph
sem_t ph_vacia;              // Indicar que está vacio el buffer de ph
sem_t recolector_finalizado; // Indicar que el hilo recolector ha finalizado

/*--- Función para procesar los FLAGS ---*/
MonitorArgumentos procesarArgumentos(int argc, char *argv[]) {
  // Inicializar la estructura con los parámetros de entrada
  MonitorArgumentos args = {.tam_buffer = -1,
                            .arch_temperatura = NULL,
                            .arch_ph = NULL,
                            .pipe_nominal = NULL};
  // Definir las flags
  char *flags[] = {"-b", "-t", "-h", "-p"};
  ...
  return args; // Retorna la estructura actualizada con todos los argumentos
               // recibidos
}
  ...
  while (1) {
    ...
    if (read(pipe, &datosSensor, sizeof(DatosSensor)) == sizeof(DatosSensor)) { // Lectura del pipe
      if (datosSensor.tipo_sensor == 1) { // Si la medición es de tipo temperatura se valida
        ...
        if (medicionTemp >= 20.0 && medicionTemp <= 31.60) { // Validar que esté dentro del rango
          sem_wait(&buffer_mutex);   // Garantizar exlcusión mutua
          if (cont_temperatura < MAX_BUFFER_SIZE) {
            temperatura_buffer[cont_temperatura] = medicionTemp;   // Guardar la medición
            cont_temperatura++; // Pasar a la siguiente medición de temperatura
        ...
        if (medicionPh >= 6.0 && medicionPh <= 8.0) {   // Validar que esté dentro del rang
          ...
          if (cont_ph < MAX_BUFFER_SIZE) {
            ...
            cont_ph++;              // Pasar a la siguiente medición de ph
      ...
      perror("\nError al leer desde el pipe\n"); // Manejor de error al leer desde el pipe
      break;
    }
  }
  close(pipe);          // Cerrar el pipe
...
void *H_ph(char *arch_ph) {
  FILE *archivoPhFinal =
      abrirArchivo(arch_ph, "w"); // Abrir archivo modo lectura

  while (1) {
    sem_wait(&ph_completa); // Espera a que haya datos disponibles en el buffer ph
    sem_wait(&buffer_mutex); // Garantizar exclusión mutua
    ...
    sem_post(&buffer_mutex); // Permitir el acceso a otro proceso
    sem_post(&ph_vacia); // Indica que hay un espacio disponible para otra medicion
  }
...
void *H_temperatura(char *arch_temperatura) {
  FILE *archivoTempFinal = abrirArchivo(arch_temperatura, "a"); // Abrir archivo modo lectura

  while (1) {
    sem_wait(&temperatura_completa); // Espera a que haya datos disponibles en el buffer temperatura
    sem_wait(&buffer_mutex);         // Garantizar exclusión mutua
    ...
    sem_post(&buffer_mutex);      // Permitir el acceso a otro proceso
    sem_post(&temperatura_vacia); // Indica que hay un espacio disponible para otra medicion
  }
...
int main(int argc, char *argv[]) {
  // Procesar los argumentos y extraer los valores de los flags
  MonitorArgumentos args = procesarArgumentos(argc, argv);
  ...
  if (args.tam_buffer == -1 || args.arch_temperatura == NULL ||
    ...
    fprintf(stderr,"Uso: %s -b tamaño_buffers -t archivo_temperatura -h archivo_ph -p pipe_nominal\n", argv[0]);
    exit(1);
  }
  ...
  sem_init(&temperatura_vacia, 0, buffer);
  sem_init(&ph_completa, 0, 0);
  sem_init(&ph_vacia, 0, buffer);
  ...
  sem_destroy(&temperatura_vacia);
  sem_destroy(&ph_completa);
  sem_destroy(&ph_vacia);

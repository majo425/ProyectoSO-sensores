...
SensorArgumentos procesarArgumentos(int argc, char *argv[]) {
  ...
  // Definir las flags que debe recibir el proceso
  char *flags[] = {"-s", "-t", "-f", "-p"};
  ...
  for (int i = 1; i < argc; i++) {         // Recorrido del argumento recibido
    for (int j = 0; j < cant_flags; j++) { // Buscar el argumento en el arreglo
      if (strcmp(argv[i], flags[j]) == 0) {
        if (i + 1 < argc) { // Asigna el argumento a la variable propia de la estructura
          switch (j) {
          case 0:
  ...
  return args; // Retorna la estructura actualizada con todos los argumentos recibidos
}
...
int main(int argc, char *argv[]) {
  ...
  SensorArgumentos args = procesarArgumentos(argc, argv);

  // Validar que se hayan proporcionado todos los argumentos requeridos
  if (args.tipo_sensor == -1 || args.tiempo == -1 || args.archivo == NULL ||
